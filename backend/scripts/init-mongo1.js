...
const MongoScript = async () => {
  ...
  try {
    ...
    const db = client.db("knowlegde-cove");
    ...
    await db.collection("books").insertMany(bookDB); // Insert books data
    ...
    await db.collection("users").createIndex({ membership_number: 1 });
    await db.collection("users").createIndex({ username: 1 });
    await db.collection("users").createIndex({ password: 1 });

    await db.collection("usersInfo").createIndex({ user_id: 1 });
    await db.collection("usersInfo").createIndex({ name: 1 });
    await db.collection("usersInfo").createIndex({ phone: 1 });
    await db.collection("usersInfo").createIndex({ address: 1 });
    await db.collection("usersInfo").createIndex({ dob: 1 });
    await db.collection("usersInfo").createIndex({ email: 1 });

    await db.collection("books").createIndex({ book_id: 1 });
    await db.collection("books").createIndex({ book_name: 1 });
    await db.collection("books").createIndex({ book_shortDescription: 1 });
    await db.collection("books").createIndex({ book_longDescription: 1 });
    await db.collection("books").createIndex({ book_author: 1 });
    await db.collection("books").createIndex({ book_genre: 1 });
    await db.collection("books").createIndex({ book_image_url: 1 });

    await db.collection("rooms").createIndex({ room_num: 1 });
    await db.collection("rooms").createIndex({ room_booked: 1 });
    await db.collection("rooms").createIndex({ date_time: 1 });
    await db.collection("rooms").createIndex({ booked_by: 1 });

    await db.collection("libBranches").createIndex({ branch_id: 1 });
    await db.collection("libBranches").createIndex({ branch_location: 1 });

    await db.collection("cart").createIndex({ membership_number: 1 });
    await db.collection("cart").createIndex({ book_id: 1 });

    await db.collection("booksStock").createIndex({ book_id: 1 });
    await db.collection("booksStock").createIndex({ branch_id: 1 });
    await db.collection("booksStock").createIndex({ avail_quantity: 1 });

    await db.collection("booksIssd").createIndex({ book_id: 1 });
    await db.collection("booksIssd").createIndex({ membership_number: 1 });
    await db.collection("booksIssd").createIndex({ issue_date: 1 });
    await db.collection("booksIssd").createIndex({ exp_date: 1 });
